...
export async function getSheetRows(token) {
  const url = `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/${encodeURIComponent(SHEET_NAME)}!A:F`
...
export async function saveSheetRows(token, rows) {
  // Normalize all rows to 6 columns (id, title, category, gdrivePath, description, updatedAt)
  ...
  const clearRes = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/${encodeURIComponent(SHEET_NAME)}!A:F:clear`, {
    method: 'POST',
    headers: {
      Authorization: *** ${token}`,
  ...
  const range = `${SHEET_NAME}!A1:F${normalized.length}`
...
export async function syncFromDrive(token) {
  ...
  const header = rows.length > 0 ? rows[0] : ['id', 'title', 'category', 'gdrivePath', 'description', 'updatedAt']
...
export async function handleUpload(token, params) {
  ...
  rows.push([String(maxId + 1), title, '', fileId, '', now])
