...
export async function listDriveFolder(token, folderId) {
  ...
  let pageToken = null
  do {
    let url = `https://www.googleapis.com/drive/v3/files?q='${encodeURIComponent(folderId)}'+in+parents+and+trashed=false&fields=nextPageToken,files(id,name)&pageSize=1000&supportsAllDrives=true`
...
/**
 * Sync Drive folder → Sheet.
 * - Adds rows for new files found in Drive
 * - Removes rows whose Drive file has been deleted
 * - Sets empty titles from filename
 * - Re-indexes all IDs
 */
export async function syncFromDrive(token) {
  ...
  // Build index of Drive files by ID
  const driveMap = {}
  for (const f of driveFiles) {
    driveMap[f.id] = { name: f.name, title: f.name.replace(/\.pdf$/i, '') }
  }

  if (rows.length < 2) {
    // Sheet is empty (header only or no header) — write all Drive files
    const header = rows.length === 1 ? rows[0] : ['id', 'title', 'category', 'gdrivePath', 'description']
    const newRows = [header]
    driveFiles.forEach((f, i) => {
      newRows.push([String(i + 1), f.name.replace(/\.pdf$/i, ''), '', f.id, ''])
    })
    await saveSheetRows(token, newRows)
    return
  }
  ...
  // Remove rows whose Drive file is gone, collect existing IDs
  dataRows = dataRows.filter(row => {
    const fileId = row[3]
    if (!fileId) return true // keep rows without gdrivePath
    existingIds.add(fileId)
    if (!driveMap[fileId]) return false // file deleted from Drive
    return true
  })

  // Update empty titles from Drive filename
  for (const row of dataRows) {
    const fileId = row[3]
    if (fileId && driveMap[fileId] && !row[1]) {
      row[1] = driveMap[fileId].title
    }
  }

  // Add new files from Drive not yet in sheet
  ...
  for (const f of driveFiles) {
    if (!existingIds.has(f.id)) {
      maxId++
      dataRows.push([String(maxId), driveMap[f.id].title, '', f.id, ''])
    }
  }
...
export async function handleGet(token) {
  ...
  return { success: true, data: rowsToSops(rows) }
}
